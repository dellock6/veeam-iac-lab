...
variable "iso_datastore" {
  type    = string
  description = "Datastore with ISO images"
  default = ""
}
...
##################################################################################
# SOURCE: Optimized for Ubuntu 24.04 (Noble Numbat)
##################################################################################
...
source "vsphere-iso" "linux-ubuntu-server" {
  ...
  # 24.04 works best by attaching the cloud-init config as a virtual CD
  cd_files = [
...
# Optimized for Ubuntu 24.04 + EFI Firmware
  # We use <esc> to ensure we stop the initial boot and 'c' to enter the GRUB console
boot_command = [
    "<wait5><esc><wait><esc><wait><esc><wait>", 
    "c<wait>",
  ...
  # ip_settle_timeout ensures Packer doesn't disconnect if the 24.04 
  # installer blips the network during the transition phase.
  ip_wait_timeout   = "20m"
...
build {
  ...
  # Runs the 24.04 optimized post-install script
  provisioner "shell" {
    execute_command = "echo '${var.ssh_password}' | {{ .Vars }} sudo -S -E bash '{{ .Path }}'"
    scripts         = ["scripts/post_install.sh"]
  }

  # This part only runs AFTER the script above finishes successfully
  # It generates a small JSON file with build details
