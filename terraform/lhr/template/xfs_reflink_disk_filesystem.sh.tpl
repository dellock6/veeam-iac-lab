...
# make filesystem XFS with reflink, double dollar sign in tf template as escape (to keep tf from trying to replace)
mkfs.xfs -b size=4096 -m reflink=1,crc=1 /dev/$${mydev}1
...
if ! grep -q $uuid /etc/fstab; then
  echo -e "UUID=$uuid $mountdir xfs defaults 1 1" | tee -a /etc/fstab
else
